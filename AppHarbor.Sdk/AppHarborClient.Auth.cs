...
namespace AppHarbor
{
  public partial class AppHarborClient
  {
    public static AuthInfo GetAuthInfo(string clientId, string clientSecret, string code)
    {
      ...
      Console.WriteLine("{0} to {1}/{2}", request.Method, client.BaseUrl, request.Resource);
    ...
    /// </summary>
    /// <param name="clientId">The client ID that can be obtained created at http://appharbor.com/clients. 
    /// The callback URL for the client must be set to:
    /// <code>http://localhost:ListenPort/Temporary_Listen_Addresses/</code>. 
    ...
    /// <remarks>This command will start a webserver on localhost to listen for the authorization code. The webserver is
    /// listining on the following url: <code>http://+:ListenPort/Temporary_Listen_Addresses/</code></remarks>
    ...
    private static string GetCodeFromLocalHost(string clientId, TimeSpan timeout)
    {
      ...
      string redirectUrl = string.Format("http://localhost:{0}/Temporary_Listen_Addresses/", port);

      string code;
      using (var listener = new HttpListener())
      {
        string localHostUrl = string.Format("http://+:{0}/Temporary_Listen_Addresses/", port);
